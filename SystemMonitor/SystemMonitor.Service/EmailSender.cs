            ...
            bool isHtml = false)
        {
            try
            {
                ...
                message.From.Add(new MailboxAddress(
                    $"SystemMonitor - {Environment.MachineName}",
                    cfg.From
                ));
                    ...
                    <div style='font-family:Segoe UI,Arial,sans-serif;font-size:14px;color:#333;background:#f5f5f5;padding:20px'>
                        <div style='max-width:650px;margin:auto;background:white;border-radius:8px;padding:20px;
                            ...
                            <p style='margin-top:20px;color:#888;font-size:12px;text-align:center'>
                                Alerte générée automatiquement par RomMonitor.
                            </p>
                        </div>
                    </div>";
                }
                ...
                CoreLog.Write($"[EMAIL] Envoyé : {subject}");
            }
            catch (Exception ex)
            {
                CoreLog.Write("[EMAIL] Erreur : " + ex.Message);
                ...
                try
                {
                    ...
                    File.AppendAllText(logPath,
                        $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} - SystemMonitor SMTP error: {ex.Message}{Environment.NewLine}");
                }
                catch { }
